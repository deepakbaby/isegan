"""
Reimplementing segan model as close as possible
with extra options
...
from spectralnorms import DenseSN, ConvSN1D
...
def generator(opts):
    ...
    if opts ['preemph_G']:
        enc_out = Conv1D(1, 2,kernel_initializer=kernel_init, strides=opts['preemph_stride'], 
                                   padding="same", use_bias=False, name="G_preemphlayer")(wav_in)
    else :
        enc_out = wav_in
...
def discriminator(opts):
    ...
    if opts ['preemph_D']:
        d_out1 = Conv1D(1, 2,kernel_initializer=kernel_init, strides=opts['preemph_stride'],
                                   padding="same", use_bias=False, name="D_preemphlayer1")(wav_in_clean)
        d_out2 = Conv1D(1, 2,kernel_initializer=kernel_init, strides=opts['preemph_stride'],
                                   padding="same", use_bias=False, name="D_preemphlayer2")(wav_in_noisy)
        d_out = keras.layers.concatenate([d_out1, d_out2])
    else:
    ...
    for layer_num, numkernels in enumerate(d_fmaps):
        ...
        if opts['applyinstancenorm'] :
            ...
            d_out = BatchNormalization(axis=2)(d_out)
        elif opts['applybatchrenorm'] :
            d_out = BatchRenormalization(axis=2)(d_out)
        elif opts ['applygroupnorm'] :
            d_out = GroupNormalization(axis=2, groups=16)(d_out)
    ...
    # add preemph initialization
    if opts['preemph_D']:
        D.get_layer("D_preemphlayer1").set_weights([preemph_init])
        D.get_layer("D_preemphlayer2").set_weights([preemph_init])
...
def discriminatorSN(opts):
    ...
    if opts ['preemph_D']:
        d_out1 = ConvSN1D(1, 2,kernel_initializer=kernel_init, strides=opts['preemph_stride'],
                                   padding="same", use_bias=False, name="D_preemphlayer1")(wav_in_clean)
        d_out2 = ConvSN1D(1, 2,kernel_initializer=kernel_init, strides=opts['preemph_stride'],
                                   padding="same", use_bias=False, name="D_preemphlayer2")(wav_in_noisy)
        d_out = keras.layers.concatenate([d_out1, d_out2])
    else:
        d_out = keras.layers.concatenate([wav_in_clean, wav_in_noisy])
    ...
    for layer_num, numkernels in enumerate(d_fmaps):
        if layer_num == 0 and opts['GT_init_D']:
            d_out = ConvSN1D(num_gt_filters, gt_filterlength, strides=strides, 
                        kernel_initializer=kernel_init, use_bias=use_bias, padding="same", name="gtlayer")(d_out)
        else :
            d_out = ConvSN1D(numkernels, kwidth, strides=strides, kernel_initializer=kernel_init, 
                        use_bias=use_bias, padding="same")(d_out)
    ...
    d_out = ConvSN1D(1, 1, padding="same", use_bias=use_bias, kernel_initializer=kernel_init, 
                    name='logits_conv')(d_out)
    d_out = Flatten()(d_out)
    d_out = DenseSN(1, activation='linear', name='d_output')(d_out)
    D = Model([wav_in_clean, wav_in_noisy], d_out)
